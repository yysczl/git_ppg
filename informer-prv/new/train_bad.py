    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_loader):
                ...
                # 前向传播 - 统一方式
                current_batch_size, seq_len = data.shape
                data = data.unsqueeze(-1)
                output = model(data)
                ...
                val_total_abs_diff += torch.sum(torch.abs(diff)).item()
                val_total_sq_diff += torch.sum(diff ** 2).item()
        ...
        val_mae = val_total_abs_diff / val_total_samples
        val_rmse = np.sqrt(val_total_sq_diff / val_total_samples)
...
def train_model_kfold(X, y, model_class, model_params, criterion, optimizer_class, optimizer_params,
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(X)):
        ...
        # 【修复】使用最佳模型状态进行评估，而不是训练结束后的状态
        model.load_state_dict(best_model_state)
        model.eval()
        
        # 【修复】使用样本级别的累加计算MAE和RMSE，而不是批次级别
        total_loss = 0
        total_abs_diff = 0
        total_sq_diff = 0
        total_samples = 0
        ...
        val_mae = total_abs_diff / total_samples
        val_rmse = np.sqrt(total_sq_diff / total_samples)
...
def evaluate_model(model, test_loader, criterion, device='cpu'):
    ...
    test_mae = total_abs_diff / total_samples
    test_rmse = np.sqrt(total_sq_diff / total_samples)
...
def train_final_model(X, y, model_class, model_params, criterion, optimizer_class, optimizer_params,
    ...
    for epoch in range(num_epochs):
        ...
        train_mae = total_abs_diff / total_samples
        train_rmse = np.sqrt(total_sq_diff / total_samples)
