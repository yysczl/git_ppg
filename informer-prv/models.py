...
#         tgt: [batch_size, tgt_seq_len, input_dim] - 目标序列（例如未来需要预测的序列）
#
#         如果tgt为None，则创建一个默认的目标序列。
#         """
#         # src: [batch_size, src_seq_len, input_dim]
#         # tgt: [batch_size, tgt_seq_len, input_dim]
#
#         # 如果tgt未提供，则创建一个默认的目标序列
#         if tgt is None:
#             tgt = torch.zeros(src.size(0), 1, self.input_dim).to(src.device)
#
#         # 输入投影
#         src_emb = self.input_projection(src)
#         tgt_emb = self.input_projection(tgt)
#
#         # 位置编码可以使用PyTorch内置的
