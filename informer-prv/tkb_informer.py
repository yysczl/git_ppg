...
# 导入拆分后的模块
from data_processor import load_and_preprocess_data, PupilDataset, set_seed, load_and_preprocess_pupil_data
...
def main():
    # 文件路径
    file_path = "../dataset/TKB/SadnessTKB.csv"
    ...
    # 加载和预处理数据
    X_train, y_train, X_val, y_val, X_test, y_test, scaler = load_and_preprocess_pupil_data(file_path)
    ...
    # 定义模型类型和参数
    model_type = "TCN"  # 可选: "Informer", "LSTM", "GRU", "BiLSTM", "TCN", "Transformer"
    ...
    # 创建测试数据加载器
    test_dataset = PupilDataset(X_test, y_test)
    ...
    # 绘制预测结果
    plot_predictions(predictions, targets, model_type, "tkb")
    ...
    # 保存最终模型
    model_save_path = f"tkb_{model_type}_model.pth"
