...
def parse_args():
    ...
    parser.add_argument('--baseline_model', type=str, default='lstm',
                        choices=['lstm', 'gru', 'bilstm', 'tcn',
                                 'transformer_baseline', 'informer'],
                        help='基准模型名称')
...
def load_data_new(config: ExperimentConfig, selected_emotions: List[str] = None):
    ...
    if train_mode in ['ppg_only', 'ppg_regression', 'ppg_classification',
        ...
        data = load_all_emotion_data(
            ppg_dir, prv_dir, selected_emotions, normalize=True, normalize_stress=True)
        ...
        result['stress_scaler'] = data.get(
            'stress_scaler', None)  # 保存压力标签scaler用于反归一化
    ...
    print(
        f"  情绪标签分布: {dict(zip(*np.unique(result['y_emotion'], return_counts=True)))}")
...
def train_mode_func(args, config: ExperimentConfig):
    ...
    # 获取模型配置
    model_class, model_params, model_name = get_model_and_params(
        config, X_prv is not None)
    ...
    if args.target_emotion:
        emotions_str = args.target_emotion
    elif args.emotions:
        emotions_str = args.emotions.replace(',', '-')
    else:
        emotions_str = "All"

    # 设置运行目录
    run_dir = config.log.setup_run_dir(
    ...
    if config.log.save_plots and task_type != 'classification':
        plot_training_process(
            ...
            model_name, config.log.run_plot_dir, show=False
        )

        plot_fold_comparison(fold_results, model_name,
                             config.log.run_plot_dir, show=False)
    ...
    if task_type != 'classification':
        ...
        test_tensors.append(torch.tensor(
            y_stress_test, dtype=torch.float32).reshape(-1, 1))
        ...
        if stress_scaler is not None:
            predictions_real = stress_scaler.inverse_transform(
                predictions.reshape(-1, 1)).flatten()
            targets_real = stress_scaler.inverse_transform(
            ...
            print(
                f"  压力标签范围: [{targets_real.min():.2f}, {targets_real.max():.2f}]")
        else:
...
def eval_mode_func(args, config: ExperimentConfig):
    ...
    print("\n===== 模型评估 =====")
    evaluator = Evaluator(
        model, device, task_type=task_type, train_mode=train_mode)
    ...
    if task_type != 'classification' and 'predictions' in test_metrics:
        ...
        save_path = plot_predictions(
            ...
            model_name + "_eval",
            config.log.run_plot_dir if config.log.run_dir else config.log.result_dir,
            show=True
        )
        print(f"\n预测散点图已保存到: {save_path}")
...
def ablation_mode_func(args, config: ExperimentConfig):
    ...
    # 获取基础模型配置
    model_class, base_params, model_name = get_model_and_params(
        config, X_prv is not None)
    ...
    if train_mode in ['dual_stream', 'multi_task']:
        ablation_configs['w/o Cross-Modal Attn'] = {
            'use_cross_modal_attention': False}
        ablation_configs['w/o Uncertainty Weighting'] = {
    ...
    if task_type == 'classification':
        ...
        for name, metrics in results.items():
            print(
                f"{name:<35} {metrics['val_accuracy']:>12.4f} {metrics['val_f1']:>12.4f}")
    else:
        print(f"{'配置':<35} {'MAE':>12} {'RMSE':>12}")
        print("-" * 60)
        for name, metrics in results.items():
            print(
                f"{name:<35} {metrics['val_mae']:>12.4f} {metrics['val_rmse']:>12.4f}")
...
def main():
    ...
    if args.target_emotion:
        if config.training.task_type == 'regression':
            ...
            print(
                f"警告: --target_emotion 参数仅对压力回归任务有效，当前任务类型为 {config.training.task_type}")
